...
public class Principal {
    ...
    private final String API_KEY = System.genenv("CLAVE_API");
...
//        Serie serie = new Serie(datos);
//
//        Optional<Serie> existente = repositorio.findByTituloContainsIgnoreCase(serie.getTitulo());
//
...
//    }
    ...
    private void buscarSerieWeb() {
        ...
        if (existente.isEmpty()) {
            repositorio.save(serie);
            System.out.println(datos);
            System.out.println("Agregado con éxito a la base de datos");
        } else {
            Serie serieExistente = existente.get();

            // Actualizamos solo los campos que cambian
            serieExistente.setTotalTemporadas(serie.getTotalTemporadas());
            serieExistente.setGenero(serie.getGenero());
            // ... cualquier otro campo que quieras mantener actualizado

            repositorio.save(serieExistente); // Esto hace el UPDATE
            System.out.println(datos);
